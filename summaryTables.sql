...
SELECT
    ...
    CASE
        WHEN count(*) FILTER (
            where
                p_send_time <= 0.1
        )::numeric / count(*)::numeric >= 0.95 THEN '是'
        ELSE '否'
    END AS "是否达标",
...
SELECT
    ...
    SUM(
        CASE
            WHEN epicenter_deviation_km <= 60 THEN 1
            ELSE 0
        END
    ) AS "偏差≤60km组数",
    ROUND(
    ...
    SUM(
        CASE
            WHEN epicenter_deviation_km <= 100 THEN 1
            ELSE 0
        END
    ) AS "偏差≤100km组数",
    ROUND(
    ...
    CASE
        WHEN (
            MAX(epicenter_deviation_km) <= 300
        ) THEN '是'
        ELSE '否'
    END AS "最大值不超过300km"
FROM public.p_compare_result;
...
SELECT '先报警后预警' AS "项目", COUNT(*) FILTER (
        WHERE
            p.first_jk_time > s.first_jk_time
    ) AS "先报警后预警数量"
...
SELECT
    ...
    CASE
        WHEN COUNT(*) FILTER (
            WHERE
                gal80_send_time <= 0.1
        )::numeric / NULLIF(COUNT(gal80_send_time), 0) >= 1 THEN '是'
        ELSE '否'
    END AS "是否达标",
    ROUND(
...
SELECT
    ...
    CASE
        WHEN COUNT(*) FILTER (
            WHERE
                gal120_send_time <= 0.1
        )::numeric / NULLIF(COUNT(gal120_send_time), 0) >= 1 THEN '是'
        ELSE '否'
    END AS "是否达标",
    ROUND(
...
SELECT
    ...
    CASE
        WHEN COUNT(*) FILTER (
            WHERE
                swave_judge_time_station <= 0.5
        )::numeric / COUNT(*)::numeric >= 0.7 THEN '是'
        ELSE '否'
    END AS "是否达标",
    ROUND(
        AVG(swave_judge_time_station),
        3
    ) AS "平均值",
...
FROM public.s_compare_result
WHERE
    gal80_judge_time_station IS NOT NULL;
...
FROM public.s_compare_result
WHERE
    gal120_judge_time_station IS NOT NULL;